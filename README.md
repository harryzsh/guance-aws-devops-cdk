...
上面的链路是 **单向** 的：观测云告警 → DevOps Agent 建 task → 飞书/企微通知。

如果你还想让 DevOps Agent 在 **调查过程中** 反向查询观测云（拉取相关的日志、指标、Trace、RUM、监控器、仪表板等），可以把观测云托管的 MCP Server 注册到 DevOps Agent 上。这是一个与本 CDK 栈完全独立的、在 AWS 控制台手动完成的步骤，不影响 `cdk deploy` / `cdk destroy`。

完整步骤、Header 拼接格式、工具白名单建议、安全注意事项和排错清单，见：[docs/guance-mcp-setup.md](./docs/guance-mcp-setup.md)

> 不做这步也完全不影响告警 → 通知主链路。只有当你希望 agent 主动查询观测云数据时才需要。
